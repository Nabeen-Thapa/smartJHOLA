...
changePassword.post("/changepwd", async (req, res) => {
    ...
    try {
        ...
        const hashedOtp = await bcrypt_1.default.hash(newPassword, 10);
        await getAdminRepo.update({ username }, { password: hashedOtp });
        //delete form readis
        const checkInRedis = await redisClient_1.default.keys('username:*');
        let redisData;
        ...
        if (deleteFromRedis) {
            const delResult = await redisClient_1.default.del(deleteFromRedis);
            if (delResult) {
